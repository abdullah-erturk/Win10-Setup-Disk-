...
Function Copy_Progression ($Files,$Partition,$fs) {
    ...
    $ProgressLabel.Text = "Starting file copy operation..."
    ...
    $ProgressLabel.Text = "File copy operation completed!"
...
Function Start_Process {
    ...
    $OutputTextBox.Clear()
    $OutputTextBox.AppendText("Starting process...`r`n")
    $OutputTextBox.ScrollToCaret()
    ...
    try {
        ...
        $OutputTextBox.AppendText("Creating FAT32 boot partition and marking it as active...`r`n")
        $OutputTextBox.ScrollToCaret()
        $Form.Refresh()
        ...
        $OutputTextBox.AppendText("FAT32 partition created successfully ($($PartitionSize.ToString("F2")) GB).`r`n")
        $OutputTextBox.ScrollToCaret()
        $Form.Refresh()
        ...
        $OutputTextBox.AppendText("NTFS partition created successfully ($($PartitionSize.ToString("F2")) GB).`r`n")
        $OutputTextBox.ScrollToCaret()
        $Form.Refresh()
        ...
        # Copy files or apply image
        If($Global:SetUp){
            $OutputTextBox.AppendText("Starting file copy operation...`r`n")
            $OutputTextBox.ScrollToCaret()
            $Form.Refresh()
            ...
            # Windows To Go - Version selection on main GUI
        ...
        $OutputTextBox.AppendText("You can safely remove the USB disk.`r`n")
        $OutputTextBox.ScrollToCaret()
...
$FileBrowser = New-Object System.Windows.Forms.OpenFileDialog -Property @{
    Title="Select install.wim/esd in ISO image or extracted source folder"
    Multiselect = $false
...
do {
    ...
    if ($FromDiskDrive.Count -eq 0) {
        $result = [System.Windows.Forms.MessageBox]::Show("No disk found.`n`nPlease connect or attach your disk first.", "Warning", [System.Windows.Forms.MessageBoxButtons]::RetryCancel, [System.Windows.Forms.MessageBoxIcon]::Warning)
...
2- Click the Windows ISO button to select the ISO file or the install.wim/esd file in the extracted folder.
CAUTION: The esd file will cause an error in the Windows To Go process.

3- Select the "Target USB Disk" from the drop-down menu.

4- Select the "Windows Setup Disk" or "Windows To Go" option.

5- If you selected "Windows Setup Disk", you can optionally check "Bypass Windows 11 System Requirements".

6- Optionally, you can select your own automatic installation file with the "Select Custom AutoUnattend.xml File" button.

7- Click "Create Disk" to create the Windows Setup Disk or Windows To Go environment.
"@
}
...
$SelectAutoUnattendButton = New-Object System.Windows.Forms.Button -Property @{
    ...
    Text = "Select Custom AutoUnattend.xml File"
...
$WTGListBox = New-Object System.Windows.Forms.ListBox -Property @{
    ...
    Size = New-Object System.Drawing.Size(375, 100)
    Visible = $False
}
...
$WTGSelectButton = New-Object System.Windows.Forms.Button -Property @{
    Location = New-Object System.Drawing.Point(380, 210)
    Size = New-Object System.Drawing.Size(70, 70)
    Text = "Select"
    Visible = $False
}
...
$Wintogo.Add_CheckedChanged({
    if ($Wintogo.Checked) {
        ...
        $BypassTPMCheckbox.Enabled = $Windows.Checked
        if (!$BypassTPMCheckbox.Checked) {
            $SelectAutoUnattendButton.Enabled = $True
        } else {
            $SelectAutoUnattendButton.Enabled = $False
...
$SelectAutoUnattendButton.Add_Click({
    if ($CustomAutoUnattendBrowser.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        $Global:CustomAutoUnattendPath = $CustomAutoUnattendBrowser.FileName
        $OutputTextBox.AppendText("Özel AutoUnattend.xml dosyası seçildi: $($Global:CustomAutoUnattendPath)`r`n")
        $OutputTextBox.ScrollToCaret()
    }
})
...
$OutputTextBox.AppendText("`r`nSelect ISO file and click the 'Create Disk' button.`r`n")
$OutputTextBox.ScrollToCaret()
