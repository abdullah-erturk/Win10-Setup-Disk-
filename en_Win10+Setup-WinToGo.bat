...
if ($hasInstallImage) {
    ...
    Write-Host "AutoUnattend.xml written to USB root directory." -ForegroundColor Green

    # Write the second file
    [IO.File]::WriteAllBytes($targetauto, [Convert]::FromBase64String($base64auto))
    Write-Host "auto.cmd written to USB root directory." -ForegroundColor Green
}
else {
    Write-Host "install.wim or install.esd not found, files not written." -ForegroundColor Yellow
}