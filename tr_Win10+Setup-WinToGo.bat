...
"@ -replace '\s',''
    ...
    if ($hasInstallImage) {
        ...
        $OutputTextBox.AppendText("AutoUnattend.xml ve auto.cmd dosyaları yazıldı.`r`n")
    }
    $OutputTextBox.ScrollToCaret()
}
...
$HowToText = New-Object System.Windows.Forms.RichTextBox -Property @{
    Location = New-Object System.Drawing.Point(10, 10)
    Size = New-Object System.Drawing.Size(420, 220)
    BackColor = "Gray"
    ForeColor = "White"
    ReadOnly = $True
    BorderStyle = "None"
...
5- Windows Kurulum Diski veya Windows To Go ortamı oluşturmak için "Diski Oluştur"a tıklayın.
"@
}
...
$CancelButton.Add_Click({
    if ($Global:ProcessRunning) {
        $result = [System.Windows.Forms.MessageBox]::Show("İşlemi durdurmak istediğinizden emin misiniz?", "Onay", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
        if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
            $Global:ProcessRunning = $False
            Enable_Controls
            $OutputTextBox.AppendText("İşlem kullanıcı tarafından durduruldu.`r`n")
            $OutputTextBox.ScrollToCaret()
        }
    } else {
        $Form.Close()
...
# Başlangıç mesajı
$OutputTextBox.AppendText("Hazır. ISO dosyasını seçin ve 'Diski Oluştur' butonuna tıklayın.`r`n")
$OutputTextBox.ScrollToCaret()
